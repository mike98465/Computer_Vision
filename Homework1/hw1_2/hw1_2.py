...
def process_one_image(img_path, pattern):
    ...
    #cv2.imwrite("results/RAW.jpg", img_copy)
    ...
    #img_copy1 = img_copy1 * 255 
    #cv2.imwrite("results/AWB.jpg", img_copy1)

    # #print("2. Demosaic")
    img = demosaic(img, pattern=pattern)
    
    #img_copy2 = np.copy(img) 
    #img_copy2 = img_copy2 * 255 
    #cv2.imwrite("results/Demosaic.jpg", img_copy2)

    # #print("3. Color Correction")
    img = color_correction(img, ccm_inv)

    #img_copy3 = np.copy(img) 
    #img_copy3 = img_copy3 * 255 
    #cv2.imwrite("results/Color Correction.jpg", img_copy3)

    # #print("4. from XYZ to RGB")
    img = XYZ2RGB(img)
    
    #img_copy4 = np.copy(img) 
    #img_copy4 = img_copy4 * 255 
    #cv2.imwrite("results/CIE XYZ to RGB.jpg", img_copy4)

    # #print("5. Tone Mapping")
    img = tone_mapping(img, I, B, index=tone_index, inv=False)
    
    #img_copy5 = np.copy(img) 
    #img_copy5 = img_copy5 * 255 
    #cv2.imwrite("results/Tone Mapping.jpg", img_copy5)
